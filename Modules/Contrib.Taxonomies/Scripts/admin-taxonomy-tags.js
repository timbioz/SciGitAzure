(function ($) {
    ...
    **********************************************************************/
    ...
    var createTermCheckbox = function ($wrapper, tag) {
        ...
        var checkboxHtml = "<input type=\"checkbox\" value=\"true\" checked=\"checked\" data-term=\"" + tag.toLowerCase() + "\" id=\"" + checkboxId + "\" name=\"" + checkboxName + "\" />";
        var radioHtml = "<input type=\"radio\" value=\"" + -nextIndex + "\" checked=\"checked\" data-term=\"" + tag.toLowerCase() + "\" id=\"" + checkboxId + "\" name=\"" + radioName + "\" />";
    ...
    **********************************************************************/
    var onTagsChanged = function (tagLabelOrValue, action, tag) {
        ...
        var $termCheckbox = $("input[data-term='" + tag.label.toLowerCase() + "']", $terms);
        ...
        if ($termCheckbox.length == 0 && action == "added") {
            createTermCheckbox($wrapper, tagLabelOrValue, this);
        }

        $termCheckbox.attr("checked", action == "added");

        if (singleChoice && action == "added") {
            $tagIt.tagit("fill", [tagLabelOrValue]);
        }

        if (action == "added") {
            if (!singleChoice) {
                var $option = $("select.term-picker", $wrapper).find("option[data-term='" + tagLabelOrValue.toLowerCase() + "']");
                $option.remove();
            }
        } else {
            $("select.term-picker", $wrapper).append("<option data-term=\"" + tagLabelOrValue.toLowerCase() + "\">" + tagLabelOrValue + "</option>");
        }
